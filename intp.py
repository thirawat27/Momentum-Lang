# --- Momentum Interpreter in Python (Version 1.0) ---
...
class ParserError(MomentumError):
    ...
    def __init__(self, message, token):
        ...
        super().__init__(f"[บรรทัดที่ {line}] ข้อผิดพลาดทางไวยากรณ์: {message}")
        self.token = token
...
class InterpreterError(MomentumError):
    ...
    def __init__(self, message, node):
        line = '?'
        if node and hasattr(node, 'token') and hasattr(node.token, 'line'):
...
class Parser:
    ...
    def statement(self):
        ...
        if tok_type == 'ID':
            ...
            # If it's a function call (parsed as ArrayAccess), it's a valid standalone statement
...
class MomentumToPythonTranspiler:
    ...
    def generic_visit(self, node): 
        raise InterpreterError(f"คำสั่งประเภท '{type(node).__name__}' ไม่รองรับภายใน JIT_FUNCTION (รองรับเฉพาะการคำนวณ, IF, FOR, RETURN)", node)
...
class Interpreter:
    ...
    # --- NEW: Matrix operation helpers ---
    def _matrix_add_sub(self, A, B, op, node):
        if not isinstance(A, list) or not isinstance(B, list) or not A or not B or not isinstance(A[0], list) or not isinstance(B[0], list):
            self.error("การบวกลบเมทริกซ์สามารถทำได้กับอาร์เรย์ 2 มิติเท่านั้น", node)
        
        rowsA, colsA = len(A), len(A[0])
        rowsB, colsB = len(B), len(B[0])

        if rowsA != rowsB or colsA != colsB:
            self.error(f"ขนาดของเมทริกซ์ไม่เท่ากัน ({rowsA}x{colsA} vs {rowsB}x{colsB})", node)
        ...
        for i in range(rowsA):
            for j in range(colsA):
                val_a = A[i][j]
                val_b = B[i][j]
                self._check_numeric_operands(node, val_a, val_b)
                C[i][j] = val_a + val_b if op == 'PLUS' else val_a - val_b
        return C

    def _scalar_mul(self, matrix, scalar, node):
        if not isinstance(matrix, list) or not matrix or not isinstance(matrix[0], list):
            self.error("การคูณด้วยสเกลาร์สามารถทำได้กับอาร์เรย์ 2 มิติเท่านั้น", node)
        if not isinstance(scalar, (int, float)):
             self.error("ตัวคูณสเกลาร์ต้องเป็นตัวเลข", node)

        rows, cols = len(matrix), len(matrix[0])
        C = [[0 for _ in range(cols)] for _ in range(rows)]
        for i in range(rows):
            for j in range(cols):
                self._check_unary_numeric(node, matrix[i][j])
                C[i][j] = matrix[i][j] * scalar
        return C
    ...
    async def visit_BinOp(self, node):
        ...
        # --- MODIFIED: Handle short-circuiting for logic ops first ---
        ...
        is_left_list = isinstance(left, list) and left and isinstance(left[0], list)
        is_right_list = isinstance(right, list) and right and isinstance(right[0], list)

        if op_type in ('PLUS', 'MINUS') and is_left_list and is_right_list:
            return self._matrix_add_sub(left, right, op_type, node)
        
        if op_type == 'MUL':
            if is_left_list and not is_right_list:
                return self._scalar_mul(left, right, node)
            if not is_left_list and is_right_list:
                return self._scalar_mul(right, left, node)
        # --- END NEW ---
    ...
    def get_variable(self, name, node):
        ...
        # Allow functions to access global scope
    ...
    async def get_array_element(self, array, indices, node):
        ...
        for index in indices:
            index = int(index) # Ensure index is integer
    ...
    async def visit_FuncCall(self, node):
        ...
        if func_name in BUILTIN_FUNCTIONS: 
            try:
                ...
                raise InterpreterError(f"เกิดข้อผิดพลาดในการเรียกฟังก์ชัน '{node.name}': {e}", node)
    ...
    async def visit_Dim(self, node):
        ...
        def create_nested_list(dimensions):
            ...
            return [create_nested_list(dimensions[1:]) for _ in range(int(dimensions[0]))]
        ar.members[var_name] = create_nested_list(dims)

    async def visit_ArrayAccess(self, node):
        # Note: In this language, the `name(args)` syntax is used for both
        # 1. Function Calls
        # 2. Array Access
        # This code first checks if 'name' is an array. If so, it performs access.
        # If it's not an array or the variable doesn't exist, it's treated as a function call.
        name = node.name_token.value
        try:
            target = self.get_variable(name, node)
        except InterpreterError:
            target = None # Variable not found, that's okay, might be a function
        ...
        # Not a list, so treat as a function call
    ...
    async def visit_Read(self, node):
        for var_node in node.variables:
            ...
            # Create a temporary node to hold the value for assignment
            if isinstance(value, (int, float)):
                temp_value_node = Num(Token("VALUE", value, var_node.token.line))
            else:
                temp_value_node = String(Token("STRING", str(value), var_node.token.line))

            assign_node = Assign(left=var_node, op=Token('ASSIGN', '=', var_node.token.line), right=temp_value_node)
    ...
    # --- Corrected RUN ASYNC implementation ---
    async def visit_RunAsync(self, node):
        """
        Schedules async functions to run concurrently in the background
        without blocking the main program flow, adhering to the "fire-and-forget" principle.
        """
        for task_expr in node.tasks:
            if not isinstance(task_expr, ArrayAccess):
                self.error("RUN ASYNC สามารถใช้ได้กับการเรียกฟังก์ชัน async เท่านั้น", task_expr)

            func_call = FuncCall(task_expr.name_token, task_expr.index_exprs)
            coro = await self._create_async_coroutine(func_call)
            
            # Create and schedule the task. The event loop will run it in the background.
            asyncio.create_task(coro)
            
        # No `await asyncio.gather()` here, allowing the main program to continue immediately.
...
# --- NEW: Matrix and OS Built-in functions ---
def builtin_mat_mul(A, B):
    """Performs matrix multiplication (dot product)."""
    if not isinstance(A, list) or not isinstance(B, list) or not A or not B or not isinstance(A[0], list) or not isinstance(B[0], list):
        raise MomentumError("mat_mul ต้องการอาร์เรย์ 2 มิติสองตัว", None)
    
    rowsA, colsA = len(A), len(A[0])
    rowsB, colsB = len(B), len(B[0])

    if colsA != rowsB:
        raise MomentumError(f"ขนาดเมทริกซ์ไม่สามารถคูณกันได้ (A's columns {colsA} != B's rows {rowsB})", None)
...
def builtin_rows(matrix):
    """Returns the number of rows in a 2D array."""
    if not isinstance(matrix, list) or not matrix:
        return 0
    return len(matrix)

def builtin_cols(matrix):
    """Returns the number of columns in a 2D array."""
    if not isinstance(matrix, list) or not matrix or not isinstance(matrix[0], list):
        return 0
    return len(matrix[0])

def builtin_os_list_dir(path="."):
    """Lists files and directories in a path."""
    try:
        return os.listdir(path)
    except Exception as e:
        raise MomentumError(f"ไม่สามารถอ่านไดเรกทอรี '{path}': {e}", None)

def builtin_os_exists(path):
    """Checks if a file or directory exists."""
    return 1 if os.path.exists(path) else 0

def builtin_os_is_file(path):
    """Checks if a path is a file."""
    return 1 if os.path.isfile(path) else 0

def builtin_os_is_dir(path):
    """Checks if a path is a directory."""
    return 1 if os.path.isdir(path) else 0

def builtin_os_make_dir(path):
    """Creates a directory."""
    try:
        os.makedirs(path, exist_ok=True)
        return 1
    except Exception as e:
        raise MomentumError(f"ไม่สามารถสร้างไดเรกทอรี '{path}': {e}", None)

def builtin_os_remove_file(path):
    """Removes a file."""
    try:
        os.remove(path)
        return 1
    except Exception as e:
        raise MomentumError(f"ไม่สามารถลบไฟล์ '{path}': {e}", None)

def builtin_os_rename(src, dst):
    """Renames a file or directory."""
    try:
        os.rename(src, dst)
        return 1
    except Exception as e:
        raise MomentumError(f"ไม่สามารถเปลี่ยนชื่อ '{src}' เป็น '{dst}': {e}", None)

def builtin_os_cwd():
    """Gets the current working directory."""
    return os.getcwd()

# Main dictionary of all built-in functions
BUILTIN_FUNCTIONS = {
    # General
    'time': time.time, 'str': str, 'int': int, 'float': float, 'len': len, 'abs': abs, 
    'round': round, 'type': builtin_type, 'sqrt': math.sqrt, 
    # Graphics
    ...
    # Matrix functions
    'mat_mul': builtin_mat_mul, 'rows': builtin_rows, 'cols': builtin_cols,
    # OS functions
    'os_list_dir': builtin_os_list_dir, 'os_exists': builtin_os_exists,
    'os_is_file': builtin_os_is_file, 'os_is_dir': builtin_os_is_dir,
    'os_make_dir': builtin_os_make_dir, 'os_remove_file': builtin_os_remove_file,
    'os_rename': builtin_os_rename, 'os_cwd': builtin_os_cwd
}
...
def run_momentum(code):
    ...
    try:
        ...
        raise # Re-raise to be caught by momentum.py
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if source_code:
        ...
        try:
            ...
            # Error is already printed, just exit cleanly
            sys.exit(1)
        print("---------------------------")